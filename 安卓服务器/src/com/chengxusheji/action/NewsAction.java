...
public class NewsAction extends BaseAction {
    ...
    public String QueryNews() {
        ...
        List<News> newsList = newsDAO.QueryNewsInfo(title, newsDate, currentPage);
        /*计算总的页数和总的记录数*/
        newsDAO.CalculateTotalPageAndRecordNumber(title, newsDate);
        /*获取到总的页码数目*/
    ...
    public String QueryNewsOutputToExcel() { 
        if(title == null) title = "";
        if(newsDate == null) newsDate = "";
        List<News> newsList = newsDAO.QueryNewsInfo(title,newsDate);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "News信息记录"; 
        String[] headers = { "标题","日期","来源","内容"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<newsList.size();i++) {
        	News news = newsList.get(i); 
        	dataset.add(new String[]{news.getTitle(),new SimpleDateFormat("yyyy-MM-dd").format(news.getNewsDate()),news.getComeFrom(),news.getNewsContent()});
        }
        /*

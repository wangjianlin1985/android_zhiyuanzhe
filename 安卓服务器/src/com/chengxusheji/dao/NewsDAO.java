...
public class NewsDAO {
    ...
    public ArrayList<News> QueryNewsInfo(String title,String newsDate,int currentPage) { 
    	Session s = factory.getCurrentSession();
    ...
    @Transactional(propagation=Propagation.NOT_SUPPORTED)
    public ArrayList<News> QueryNewsInfo(String title,String newsDate) { 
    	Session s = factory.getCurrentSession();
    	String hql = "From News news where 1=1";
    	if(!title.equals("")) hql = hql + " and news.title like '%" + title + "%'";
    	if(!newsDate.equals("")) hql = hql + " and news.newsDate like '%" + newsDate + "%'";
    ...
    /*计算总的页数和记录数*/
    @Transactional(propagation=Propagation.NOT_SUPPORTED)
    public void CalculateTotalPageAndRecordNumber(String title,String newsDate) {
        Session s = factory.getCurrentSession();
        String hql = "From News news where 1=1";
        if(!title.equals("")) hql = hql + " and news.title like '%" + title + "%'";
        if(!newsDate.equals("")) hql = hql + " and news.newsDate like '%" + newsDate + "%'";

...
public class ExerciseEditActivity extends Activity {
			...
			public void onClick(View v) {
				try {
					...
					if(ET_exerciseName.getText().toString().equals("")) {
						Toast.makeText(ExerciseEditActivity.this, "活动名称输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_serviceTime.getText().toString().equals("")) {
						Toast.makeText(ExerciseEditActivity.this, "服务时长输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_address.getText().toString().equals("")) {
						Toast.makeText(ExerciseEditActivity.this, "活动地点输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_personNum.getText().toString().equals("")) {
						Toast.makeText(ExerciseEditActivity.this, "参与人数输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_content.getText().toString().equals("")) {
						Toast.makeText(ExerciseEditActivity.this, "活动内容输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传活动信息*/
					ExerciseEditActivity.this.setTitle("正在更新活动信息，稍等...");
	...
	private void initViewData() {
		...
		this.TV_exerciseId.setText(exerciseId+"");
		this.ET_exerciseName.setText(exercise.getExerciseName());
		Date exerciseDate = new Date(exercise.getExerciseDate().getTime());
		this.dp_exerciseDate.init(exerciseDate.getYear() + 1900,exerciseDate.getMonth(), exerciseDate.getDate(), null);
		this.ET_serviceTime.setText(exercise.getServiceTime());
		this.ET_address.setText(exercise.getAddress());
		this.ET_personNum.setText(exercise.getPersonNum() + "");
		this.ET_content.setText(exercise.getContent());
		for (int i = 0; i < teamList.size(); i++) {
			if (exercise.getTeamObj().equals(teamList.get(i).getTeamUserName())) {
				this.spinner_teamObj.setSelection(i);
				break;
			}
		}
	}
